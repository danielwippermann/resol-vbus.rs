...
mod tests {
    ...
    fn test_write_live_data() {
...

        {
            ...
            let data = &[170, 16, 0, 17, 126, 16, 0, 1, 27, 52, 56, 34, 56, 34, 5, 70, 61, 126, 121, 127, 14, 62, 56, 34, 56, 34, 5, 70, 56, 34, 56, 34, 5, 70, 56, 34, 56, 34, 5, 70, 2, 2, 56, 34, 4, 29];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();

            let start_timestamp = UTC.timestamp(1486857602, 283000000);
            let end_timestamp = UTC.timestamp(1486857602, 284000000);
            let data = &[15, 39, 15, 39, 0, 19, 15, 39, 70, 5, 0, 126, 15, 39, 15, 39, 0, 19, 15, 39, 15, 39, 0, 19, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();

            let start_timestamp = UTC.timestamp(1486857602, 474000000);
            let end_timestamp = UTC.timestamp(1486857602, 474000000);
            let data = &[0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 15, 39, 15, 39, 0, 19, 15, 39, 15, 39, 0, 19];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();

            let start_timestamp = UTC.timestamp(1486857602, 683000000);
            let end_timestamp = UTC.timestamp(1486857602, 704000000);
            let data = &[0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 16, 14, 0, 0, 0, 97, 1, 6, 0, 0, 0, 120, 0, 0, 0, 0, 0, 127];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();
            ...
            let data = &[170, 21, 0, 17, 126, 16, 0, 1, 10, 64, 2, 10, 0, 0, 0, 115, 56, 34, 56, 34, 5, 70, 0, 0, 0, 0, 0, 127, 1, 11, 0, 0, 0, 115, 1, 6, 0, 0, 0, 120];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();

            let start_timestamp = UTC.timestamp(1486857603, 314000000);
            let end_timestamp = UTC.timestamp(1486857603, 314000000);
            let data = &[4, 8, 0, 0, 0, 115, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();

            let start_timestamp = UTC.timestamp(1486857603, 534000000);
            let end_timestamp = UTC.timestamp(1486857603, 535000000);
            let data = &[170, 85, 102, 17, 126, 16, 0, 2, 14, 21, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();

            let start_timestamp = UTC.timestamp(1486857603, 763000000);
            let end_timestamp = UTC.timestamp(1486857603, 764000000);
            let data = &[0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127, 0, 0, 0, 0, 0, 127];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();
            ...
            let data = &[170, 16, 0, 17, 126, 16, 0, 1, 27, 52, 56, 34, 56, 34, 5, 70, 60, 126, 121, 127, 14, 63, 56, 34, 56, 34, 5, 70, 56, 34, 56, 34, 5, 70, 56, 34, 56, 34, 5, 70, 2, 2, 56, 34, 4, 29];
            ldrw.write_raw_data(start_timestamp, end_timestamp, data).unwrap();
